<template>
  <div class="row" :class="{ 'd-none': is_deleted }">
    <div class="p-3 my-2 border rounded-3">
      <div class="ms-3 mb-3 text-muted">
        <i v-if="retweeted_by === $store.state.username">Retweeted by You</i>
        <i v-else>Retweeted by {{ retweeted_by }}</i>
      </div>
      <div>
        <div class="d-flex my-1">
          ...
          <div class="card ms-1 w-100 border-0" @click="push_to_tweet_thread">
          ...
          <!-- Delete tweet modal -->
          <div class="modal" :id="`delete-${unique_id}`" data-bs-backdrop="static" data-bs-keyboard="false"
...
export default {
  ...
  created() {
    ...
    this.is_owner = this.$store.state.username == this.retweeted_by;
