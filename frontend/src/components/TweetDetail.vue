    ...
    <div class="d-flex p-3 my-2 border rounded-3">
      ...
      <div class="card ms-1 w-100 border-0">
        ...
        <div class="card-footer text-muted d-flex justify-content-around border-0 bg-white">
          ...
          <div>
            <i @click="repost_tweet" class="bi interactions"
...
export default {
  ...
  methods: {
    get_tweet_replies(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/replies/`,
      })
        .then((response) => {
    ...
    get_retweet_count(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/retweet-count/`,
      })
        .then((response) => {
    ...
    get_quote_count(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/quote-count/`,
      })
        .then((response) => {
    ...
    like_tweet() {
      axios({
        method: "post",
        url: `http://localhost:8000/tweets/${this.id}/like/`,
      })
        .then(() => {
          this.get_like_status((this.id));
          this.get_dislike_status((this.id))
          this.get_like_count(this.id)
          this.get_dislike_count(this.id)
        })
        .catch((error) => {
          console.log(error);
        });
    ...
    dislike_tweet() {
      axios({
        method: "post",
        url: `http://localhost:8000/tweets/${this.id}/dislike/`,
      })
        .then(() => {
          this.get_like_status((this.id));
          this.get_dislike_status((this.id))
    ...
    get_like_status(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/like-status/`,
      })
        .then((response) => {
    ...
    get_dislike_status(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/dislike-status/`,
      })
        .then((response) => {
    ...
    get_quote_status(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/quote-status/`,
      })
        .then((response) => {
    ...
    get_retweet_status(tweet_id) {
      axios({
        method: "get",
        url: `http://localhost:8000/tweets/${tweet_id}/retweet-status/`,
      })
        .then((response) => {
    ...
    delete_tweet() {
      axios({
        method: "delete",
        url: `http://localhost:8000/tweets/${this.id}/delete/`,
      })
        .then(() => {
  ...
  created() {
    this.get_like_count(this.id)
    this.get_dislike_count(this.id)
    this.get_reply_count(this.id)
    this.get_repost_count(this.id)
    this.get_retweet_count(this.id)
    this.get_quote_count(this.id)

    this.is_liked = this.get_like_status(this.id);
    this.is_disliked = this.get_dislike_status(this.id);
    this.is_quoted = this.get_quote_status(this.id)
    this.is_retweeted = this.get_retweet_status(this.id)
    ...
    this.get_tweet_replies(this.id)
  },
};
</script>
